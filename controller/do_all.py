...
def process_user(input_base, output_base, user):
    ...
    create_globals.process_all(DATA_BASE_DIR, REMOTE_DIR)
	
    remote_file = REMOTE_DIR + "/" + sys.argv[1] + ".zip"
    target_dir = os.path.join(REMOTE_DIR, sys.argv[1])
    ...
    move_file('comparisons')
    move_file('curves')
    move_file('dictionaries')
    move_file('users')
...
def move_file(filename):
	...
	transport.connect(username = USERNAME, password = PASSWORD) #pkey = mykey)
