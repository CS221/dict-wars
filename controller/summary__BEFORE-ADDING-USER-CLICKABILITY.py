...
def transfer_user(input_base, output_base, user):
    ...
    # And, transfer the files over.
    try:
        shutil.rmtree(output_dir, True)  # Remove the directory, ignoring errors.
        shutil.copytree(input_dir, output_dir)
    except shutil.Error:
        print "Problem copying directory for %s; problem was %s." % (user, sys.exc_info())
        return None

    # Find and parse the comparison file.
    values = []
    filename = os.path.join(output_dir, COMPARISONS_FILE)
    ...
    values = []
    filename = os.path.join(output_dir, GENERATORS_FILE)
    try:
        with open(filename, "r") as input:
            values = input.readlines()
    except IOError:
